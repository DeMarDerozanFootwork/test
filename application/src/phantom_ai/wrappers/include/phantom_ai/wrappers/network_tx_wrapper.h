...
//#if defined(ENABLE_GSTREAMER)
    ...
    bool sendEncodedImageFrame(std::vector<phantom_ai::CameraID>& cam_location, double ts, uint32_t frame_count);
    ...
    void registerEncodingMosaicImg(std::function<int(uint32_t, uint8_t*, uint32_t, uint32_t*, uint32_t*, uint32_t*)> callback_func);
//#endif
...
//#if defined(ENABLE_GSTREAMER)
    std::function<int(uint32_t, uint8_t*, uint32_t, uint32_t*, uint32_t*, uint32_t*)> encoding_mosaicImg_gst_callback_func_;
//#endif
    // Stats
    ...
    uint32_t mosaicImg_width_ = 0;
    uint32_t mosaicImg_height_= 0;
#endif
  };
