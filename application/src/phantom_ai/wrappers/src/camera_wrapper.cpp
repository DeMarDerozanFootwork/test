...
#if defined(ENABLE_GSTREAMER)
    PHANTOM_ERROR(" ++++++++++  src/phantom_ai/wrappers/src/camera_wrapper.cpp  ENABLE_GSTREAMER ON ");//WOOSEOK
#else
    PHANTOM_ERROR(" ----------  src/phantom_ai/wrappers/src/camera_wrapper.cpp  ENABLE_GSTREAMER OFF ");//WOOSEOK
#endif
...
#if 0
  void CameraWrapper::initGstEncoder(phantom_ai::NetTxWrapper* net_tx_wrapper)
  {
    if (!hw_cam_ || !net_tx_wrapper)
    {
      PHANTOM_ERROR("CameraWrapper or NetTxWrapper not initialized, cannot setup GStreamer encoder.");
      return;
    }

    uint32_t mosaic_width = 0;
    uint32_t mosaic_height = 0;

    getMosaicDimensions(mosaic_width, mosaic_height);
    bool success = net_tx_wrapper->initGstEncoder(mosaic_width, mosaic_height);
    if (success)
    {
      auto encoder_callback = NetTxWrapper::getFrameFeedCallback();
      registerEncoderCallback(encoder_callback);
      PHANTOM_LOG("CameraWrapper: Successfully connected camera pipeline to GStreamer encoder.");
    }
    else
    {
      PHANTOM_ERROR("CameraWrapper: Failed to initialize and connect GStreamer encoder.");
    }
  }
#endif
//#endif
