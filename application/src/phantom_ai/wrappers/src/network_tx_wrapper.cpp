...
#define IMAGE_DEFAULT_SIZE ((1024 * 1024 * 3) / 2 )
#endif
...
namespace phantom_ai
{
  ...
  bool NetTxWrapper::sendImageFrame(std::vector<cv::Mat>& img_arr, std::vector<phantom_ai::CameraID>& cam_location, double ts, uint32_t frame_count)
  {
    ...
    bool success = sendRawFrame(frame_);
  ...
  void NetTxWrapper::registerEncodingMosaicImg(
    std::function<int(uint8_t*, uint32_t, uint32_t*, uint32_t*, uint32_t*, uint32_t*)> callback_func)
  {
    encoding_mosaicImg_gst_callback_func_ = callback_func;
  }

  bool NetTxWrapper::sendEncodedImageFrame(std::vector<phantom_ai::CameraID>& cam_location, double ts)
  {
    ...
    std::vector<uint8_t> encoded_data_buffer(IMAGE_DEFAULT_SIZE);
    uint32_t sdk_frame_number = 0;
    uint32_t encoded_size = 0;
    uint32_t mosaic_w = 0, mosaic_h = 0;

    int result = get_latest_encoded_imgMosaicObj(encoded_data_buffer.data(), encoded_data_buffer.size(), &encoded_size, &sdk_frame_number, &mosaic_w, &mosaic_h);
    if (result != 0) {
        PHANTOM_ERROR("Encoded frame %d not found or buffer too small", sdk_frame_number);
        return false;
    }
printf("==== sendEncodedImageFrame -> get_latest_encoded_imgMosaicObj result was %s\n",(!result)?"OK":"FAILED!");
    encoded_data_buffer.resize(encoded_size);

    uint32_t offset = 0;
    memset(&frame_.header, 0, sizeof(EmbeddedNet_PhantomHeader));
    frame_.header.timestamp = static_cast<uint64_t>(ts * SEC2USEC);
    frame_.header.img_frame_count = sdk_frame_number;
...
printf("==== sendEncodedImageFrame : encoded_data_buffer %zu  vs frame_.data_size %u \n",encoded_data_buffer.size(), frame_.data_size);
    uint8_t *dest = (&frame_.data[0]);
    if (encoded_data_buffer.size() > frame_.data_size) {
        PHANTOM_ERROR("Encoded data size (%zd) is larger than the network buffer (%d)!", encoded_data_buffer.size(), frame_.data_size);
        data_buf_mutex_.unlock();
        return false;
    }
    ...
    bool success = sendRawFrame(frame_);
    data_buf_mutex_.unlock();
printf("_________ sendEncodedImageFrame with frame %d, %zd byte was %s\n", sdk_frame_number, encoded_data_buffer.size(), (success)?"good":"failed");
    return success;
  }
#endif
  ...
  void NetTxWrapper::resetFrameBuffer(const bool vision_only_dual_logging, const bool init, const std::vector<int>& annotation_image_widths)
  {
    frame_.data_size = 0;
      ...
      frame_.data_size += IMAGE_DEFAULT_SIZE;printf("++++ resetFrameBuffer : image siz is %u  ",frame_.data_size);
#endif
...
printf("  And total frame_.data_size is configured %u \n",frame_.data_size);
    // Allocate new memory
    frame_.data = new uint8_t[frame_.data_size];
