...
//* Initialize Image encoding by Gst for logging images
      ...
      gst_h264encoding_enabled_ = phantom_ai::get_yaml_key<bool>(gst_cfg, "encoding_image_by_h264", false);
      ...
      if(gst_h264encoding_enabled_)
      {
        ...
        if (gst_init_status) {
            ...
            net_tx_wrapper_->registerEncodingMosaicImg(&get_latest_encoded_imgMosaicObj);
...
#if defined(ENABLE_GSTREAMER)
      if(gst_h264encoding_enabled_) {
        net_tx_wrapper_->sendEncodedImageFrame(loc_arr, output_arr->timestamp);
      }
#else
